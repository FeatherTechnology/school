...
?>
    ...
    <table class="table table-bordered" id="show_student_scholarship_list">
        ...
        <tbody>
            ...
            $getStudentListQry = $connect->query("SELECT DISTINCT 
    sc.admission_number, 
    sc.student_name, 
    std.standard, 
    sh.section, 
    ...
    fc.remark, 
    (COALESCE(afd.scholarship, 0)) AS fees_schol
FROM
    `student_creation` sc
...
WHERE 
    af.academic_year = '$academicyear'  AND fc.academic_year  = '$academicyear'
    AND sc.medium = '$stdMedium' 
    AND sh.standard = '$stdStandard' 
    AND ('$stdSection' = '0' OR sh.section = '$stdSection') 
    AND sc.status = '0' 
AND afd.fees_table_name NOT IN ('transport', 'extratable', 'amenitytable') -- Check for admission_fees_details
    AND (fc.fees_table_name NOT IN ('transport', 'extratable', 'amenitytable') OR fc.fees_table_name IS NULL) 
    AND sc.school_id = '$school_id'
    AND (COALESCE(fc.scholarship_amount, 0) > 0 OR COALESCE(afd.scholarship, 0) > 0) -- Only students with scholarship or concession
ORDER BY sc.admission_number ASC;
");  // Add this ORDER BY clause

            $i = 1;
            $schoolScholarshipTotal = 0;
            while ($studentList = $getStudentListQry->fetchObject()) {
                $overallConcession = (float)$studentList->total_scholarship_amount + (float)$studentList->fees_schol;
            ?>
                <tr>
                    <td><?php echo $i++; ?></td>
                    ...
                    <td><?php echo $overallConcession; ?></td>
                    <td><?php echo $studentList->remark; ?></td>
                </tr>
            <?php
                $schoolScholarshipTotal += $overallConcession;
            } ?>
        </tbody>
        <tfoot>
...
?>
    <table class="table table-bordered" id="show_student_scholarship_list">
        ...
        <tbody>
            ...
            <?php
            $getStudentListQry = $connect->query("SELECT DISTINCT sc.admission_number, sc.student_name, std.standard, sh.section, sc.sms_sent_no,  (COALESCE(fc.scholarship_amount, 0)) AS total_scholarship_amount,
(COALESCE(afd.scholarship, 0)) AS fees_schol,
CASE 
...
WHERE afs.academic_year = '$academicyear' && fc.academic_year = '$academicyear' && sc.medium = '$stdMedium' &&  sh.standard = '$stdStandard' && ($stdSection = '0' OR sh.section = '$stdSection') && sc.status = '0' && fc.fees_table_name != 'transport' && fc.fees_table_name != 'grptable' && afd.fees_table_name != 'transport' && afd.fees_table_name != 'grptable'  && sc.school_id = '$school_id' AND (COALESCE(fc.scholarship_amount, 0) > 0 OR COALESCE(afd.scholarship, 0) > 0)");
            $i = 1;
            $grnd_total_amount = 0;
            while ($studentList = $getStudentListQry->fetchObject()) {
                $overallConcession = (float)$studentList->total_scholarship_amount + (float)$studentList->fees_schol;
            ...
            <?php
                $grnd_total_amount += $overallConcession;
            } ?>
        </tbody>
        <tfoot>
            <tr style="font-weight: bold;">
...
LEFT JOIN extra_curricular_activities_fee ecaf ON ecaf.extra_fee_id = fc.fees_id AND fc.fees_table_name = 'extratable'
LEFT JOIN amenity_fee af ON af.amenity_fee_id = fc.fees_id AND fc.fees_table_name = 'amenitytable'
JOIN admission_fees afs ON
    sc.student_id = afs.admission_id
LEFT JOIN admission_fees_details afd ON afd.admission_fees_ref_id = afs.id
LEFT JOIN area_creation_particulars acp ON acp.particulars_id = fc.fees_id AND fc.fees_table_name = 'transport' 
WHERE afs.academic_year = '$lastyear' && sc.medium = '$stdMedium' &&  sh.standard = '$stdStandard' && ($stdSection = '0' OR sc.section = '$stdSection') && sc.status = '0' && sc.school_id = '$school_id' AND (COALESCE(fc.scholarship_amount, 0) > 0 OR COALESCE(afd.scholarship, 0) > 0)");
            $i = 1;
...
?>
    ...
    <table class="table table-bordered" id="show_student_scholarship_list">
        ...
        <tbody>
            ...
            $getStudentListQry = $connect->query("SELECT DISTINCT sc.admission_number, sc.student_name, std.standard, sh.section, sc.sms_sent_no, (COALESCE(fc.scholarship_amount, 0)) AS total_scholarship_amount, acp.particulars AS concession_fee_name, fc.remark,  (COALESCE(afd.scholarship, 0)) AS fees_schol
FROM `student_creation` sc 
JOIN student_history sh ON
    sc.student_id = sh.student_id AND sh.academic_year = '$academicyear'
JOIN standard_creation std ON sh.standard = std.standard_id
...
WHERE fc.academic_year = '$academicyear' && sc.medium = '$stdMedium' &&  sh.standard = '$stdStandard' && ($stdSection = '0' OR sc.section = '$stdSection') && sc.status = '0' && fc.fees_table_name = 'transport' && sc.school_id = '$school_id' AND (COALESCE(fc.scholarship_amount, 0) > 0 OR COALESCE(afd.scholarship, 0) > 0) ");
            $i = 1;
            $grnd_total_fee = 0;
            while ($studentList = $getStudentListQry->fetchObject()) {
                $overallConcession = (float)$studentList->total_scholarship_amount + (float)$studentList->fees_schol;
            ...
            <?php
                $grnd_total_fee += $overallConcession;
            } ?>
        </tbody>
        <tfoot>
            <tr style="font-weight: bold;">
