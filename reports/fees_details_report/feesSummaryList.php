...
<table class="table table-bordered" id="show_student_fees_summary_list" style="text-align: left;">
    <thead>
        <tr>
            <th colspan='12' class="report-title">Fees Summary Report From <?php echo $feesFromDate->format('d-m-Y'); ?> To <?php echo $feesToDate->format('d-m-Y'); ?> </th>
        </tr>
        <tr>
            <th>S.No</th>
            <th>Date</th>
    ...
    <tbody>
        ...
        while ($startdate <= $feesToDate) {
            ...
            $getCollectedFeesQry = $connect->query("SELECT COALESCE(SUM(afd.fee_received),0) AS collectedFees ,afd_deno.payment_mode FROM `admission_fees` af JOIN admission_fees_details afd ON af.id = afd.admission_fees_ref_id  JOIN 
                admission_fees_denomination afd_deno ON af.id = afd_deno.admission_fees_ref_id  WHERE af.receipt_date ='$from_date' AND afd.fees_table_name ='grptable' AND af.school_id = '$school_id'    GROUP BY 
         afd_deno.payment_mode  ");
            ...
            $getBookFeesQry = $connect->query("SELECT afd_deno.payment_mode, COALESCE(SUM(afd.fee_received),0) AS bookFees FROM `admission_fees` af JOIN admission_fees_details afd ON af.id = afd.admission_fees_ref_id  JOIN 
                admission_fees_denomination afd_deno ON af.id = afd_deno.admission_fees_ref_id  WHERE af.receipt_date ='$from_date' AND afd.fees_table_name ='amenitytable' AND af.school_id = '$school_id' GROUP BY 
        afd_deno.payment_mode");
            $book_fees =0;
            $bookFeeSplit = [];
...
$eca_fees =0;
        ...
        transport_admission_fees_denomination tafd_deno ON taf.id = tafd_deno.admission_fees_ref_id  WHERE taf.receipt_date ='$from_date' AND taf.school_id = '$school_id' GROUP BY 
        ...
        last_year_fees_denomination lyfd_deno ON lyf.id = lyfd_deno.admission_fees_ref_id  WHERE lyf.receipt_date ='$from_date' AND lyf.school_id = '$school_id' GROUP BY lyfd_deno.payment_mode");            
            $lastYearFeeSplit = [];
            $lastyearFees = 0;
