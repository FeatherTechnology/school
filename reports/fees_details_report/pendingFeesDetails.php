...
WHERE sc.year_id = '$academicyear' && sc.medium = '$stdMedium' &&  sc.standard = '$stdStandard' && sc.section = '$stdSection' && sc.leaving_term !='1' && sc.leaving_term !='5'  && sc.school_id = '$school_id' ORDER BY sc.student_name ASC");
            $i = 1;
            $grnd_total_fee = 0;
            $grnd_paid_fee = 0;
            $grnd_concession = 0;
...
FROM `student_creation` sc 
JOIN standard_creation std ON sc.standard = std.standard_id
WHERE sc.year_id = '$academicyear' && sc.medium = '$stdMedium' &&  sc.standard = '$stdStandard' && sc.section = '$stdSection' && sc.leaving_term !='1' && sc.leaving_term !='5' && sc.school_id = '$school_id' && sc.extra_curricular !='' ORDER BY sc.student_name ASC");
            $i = 1;
            $grnd_total_fee = 0;
            $grnd_paid_fee = 0;
            $grnd_concession = 0;
...
FROM `student_creation` sc 
JOIN standard_creation std ON sc.standard = std.standard_id
WHERE sc.year_id = '$academicyear' && sc.medium = '$stdMedium' &&  sc.standard = '$stdStandard' && sc.section = '$stdSection' && sc.leaving_term !='1' && sc.leaving_term !='5'  && sc.school_id = '$school_id' && sc.transportarearefid !='' ORDER BY sc.student_name ASC ");
            $i = 1;
            $grnd_total_fee = 0;
            $grnd_paid_fee = 0;
            $grnd_concession = 0;
            ...
            while ($studentList = $getStudentListQry->fetchObject()) {
                ...
                $getPaidFeesQry = $connect->query("SELECT COALESCE( (SUM(tafd.fee_received) + SUM(tafd.scholarship)),0) AS paidFee, COALESCE(SUM(tafd.scholarship),0) + COALESCE((SELECT SUM(scholarship_amount) FROM fees_concession WHERE student_id ='$studentList->student_id' AND fees_table_name ='transport' AND fees_master_id = tafd.area_creation_id),0) AS concession FROM transport_admission_fees_details tafd JOIN  transport_admission_fees taf ON tafd.admission_fees_ref_id = taf.id WHERE tafd.area_creation_id = '$transport_id' && taf.academic_year = '$academicyear' && taf.admission_id ='$studentList->student_id' ");
                if ($getPaidFeesQry->rowCount() > 0) {
                    $paidFeeInfo = $getPaidFeesQry->fetch();
                    $paid_fee = $paidFeeInfo['paidFee'];
                    $concession = $paidFeeInfo['concession'];
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if ($transport_pending == '0' && $paid_fee == '0' && ($leavingTerm == '2' || $leavingTerm == '3')) {
                            ...
                            $transport_totals = ($transport_pending == '0' && $paid_fee == '0') ? $transportTotal : $transport_pending;
                        }
