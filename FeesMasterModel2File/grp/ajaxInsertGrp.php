...
if(isset($_POST['grp_ledger_name'])){
	$grp_ledger_name = $_POST['grp_ledger_name']; 
}
...
AND grp_particulars = '".$grp_particulars."' AND grp_amount = '".$grp_amount."' AND grp_ledger_name = '".$grp_ledger_name."' AND standard = '".$standard."' ");
while ($row=$selectClass->fetch_assoc()){
		...
		$grpc_ledger_name    = $row["grp_ledger_name"];
		$grp_Status  = $row["status"];
	}
if($grp_academic_year != '' && $grp_medium != '' && $grp_student_type != '' && $grp_standard != '' && $grpc_particulars != ''  && $grpc_amount != '' && $grpc_ledger_name != '' && $grp_Status == 0){ echo "1";
	...
	AND grp_particulars = '".$grp_particulars."' AND grp_amount = '".$grp_amount."' AND grp_ledger_name = '".$grp_ledger_name."' ");
	...
	if($fees_id>0){
		...
		AND grp_particulars = '".$grp_particulars."' AND grp_amount = '".$grp_amount."' AND grp_ledger_name = '".$grp_ledger_name."' WHERE fees_id='".$fees_id."' ");
		if($updateClass == true){
		    $message="Fees Details Updated Succesfully";
	    }
    }
	else{ 
	    $insertClass=$mysqli->query("INSERT INTO fees_master(academic_year,medium,student_type,standard,grp_particulars,grp_amount,grp_ledger_name,insert_login_id,grp_status) VALUES('".strip_tags($academic_year)."','".strip_tags($medium)."',
		'".strip_tags($student_type)."','".strip_tags($standard)."','".strip_tags($grp_particulars)."','".strip_tags($grp_amount)."','".strip_tags($grp_ledger_name)."','".strip_tags($insert_login_id)."', '1')");
	    if($insertClass == true){
