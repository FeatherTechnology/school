...
$query = "SELECT stdc.*, sc.standard as std_name, stdc.admission_number, stdc.facility FROM student_creation stdc JOIN student_history sh ON stdc.student_id = sh.student_id JOIN standard_creation sc ON sh.standard = sc.standard_id WHERE  stdc.school_id='$school_id' AND (stdc.status = 0 OR stdc.year_id <> '$year_id')AND sh.academic_year='$year_id'";
...
foreach ($result as $row) {
    ...
    if ($status == 1) {
        ...
        $action = ''; // No actions for inactive students
    } else {
