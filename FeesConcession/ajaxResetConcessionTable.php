...
<!-- <input type='text' id='pay_fees_id ' class='pay_fees_id ' name='pay_fees_id' value=''> -->

		<?php
				$ctselect="SELECT * FROM student_creation WHERE student_id='$student_name1' AND status=0";
				$ctresult=$mysqli->query($ctselect);
				if($ctresult->num_rows>0){
					$i=1;
					while($ct=$ctresult->fetch_assoc()){
						...
						<label style="background-color:#307ecc;color:#fff;width:100%;text-align:center" ><input style="background-color:#307ecc;color:#fff;width:100%;border:none;text-align:center" class="form-control" type="text" readonly id="student_name2" value="<?php if(isset($ct["student_name"])){ echo $ct["student_name"];}?> Fee Detail for"></label> <input type="hidden" readonly name="standard" value="<?php if(isset($ct["standard"])){ echo $ct["standard"];}?>" >
						<?php
						$ctselect1="SELECT pay_fees_id FROM pay_fees WHERE student_id='$student_name1' AND status=0";
						$ctresult1=$mysqli->query($ctselect1);
						if($ctresult1->num_rows>0){
							$i=1; 
							while($ct1=$ctresult1->fetch_assoc()){  $payfeesid =$ct1["pay_fees_id"]; ?>
	...
	<table id="general_concessionTable1" class="table custom-table" cellspacing="0" >
			...
			if (isset($payfeesid)) {
				$currentAcademicYear = $_SESSION['academic_year']; 
				$school_id =$_SESSION['school_id']; 
				$ctselect="SELECT * FROM pay_fees WHERE student_id='$student_name1' AND pay_fees_id='$payfeesid' AND academic_year='$currentAcademicYear' AND school_id = '$school_id' AND status=0";
				$ctresult=$mysqli->query($ctselect);
			if($ctresult->num_rows>0){
			$i=1;
			while($ct=$ctresult->fetch_assoc()){
				...
				<tr>
				<td hidden>
				<input type="hidden" readonly id="grp_fees_id" name="grp_fees_id[]" class="form-control" value = "0"><br></td>
				<td>
				...
				<td>
				<input type="number" id="grp_concession_amount" name="grp_concession_amount[]" class="form-control grp_concession_amount" value="0" onkeyup="getBalance(this)"><span class="tooltiptext"><?php if(isset($s_array5)){ echo $s_array5[$i]; }?></span><br>
				<input type="hidden" id="grp_concession_amount1" name="grp_concession_amount1[]" class="form-control grp_concession_amount1" value="<?php if(isset($s_array5)){ echo $s_array5[$i]; }?>" ></td>
				
				<td>
				<input type="number" readonly id="grp_balance_amount" name="grp_balance_amount[]" class="form-control grp_balance_amount" value = "<?php if(isset($s_array4)){ echo $s_array4[$i]; }?>"><br></td>
				
				<td>
				<input type="text"  id="grp_remarks" name="grp_remarks[]" class="form-control" value ="<?php if($s_array6[$i] != ''){ echo $s_array6[$i]; }else{  } ?>"><br></td>
					</tr>
				<?php } } } ?>
			<?php } else {
				// without payfeesid
			...
			while($ct=$ctresult->fetch_assoc()){
				...
				<tr>
				<td hidden>
				<input type="hidden" readonly id="grp_fees_id" name="grp_fees_id[]" class="form-control" value = "<?php if(isset($s_array5)){ echo $s_array5[$i]; }?>"><br></td>
				<td>
				...
				<td>
				<input type="number" id="grp_concession_amount" name="grp_concession_amount[]" class="form-control grp_concession_amount" value="0" onkeyup="getBalance(this)"><span class="tooltiptext">0</span><br>
				<input type="hidden" id="grp_concession_amount1" name="grp_concession_amount1[]" class="form-control grp_concession_amount1" value="0"></td>
...
<?php if (isset($payfeesid)){ 
			...
			?>
					...
					<td>
					<input type="number" readonly id="paid_fees" name="paid_fees2[]" class="form-control" value = "<?php if(isset($s_array3)){ echo $s_array3[$i]; }?>"><br>
					</td>
...
<?php }else{
		...
		$ctselect = "SELECT extra_curricular FROM student_creation WHERE student_id ='$student_name1' AND standard='$standard1' AND status=0";
			// print_r($ctselect);
		...
		<tr>
			<td hidden>
			<input type="hidden" readonly id="extra_fees_id" name="extra_fees_id[]" class="form-control" value = "<?php if(isset($s_array5)){ echo $s_array5[$i]; }?>"><br></td>
			...
			<input type="number" readonly id="extra_amount" name="extra_amount[]" class="form-control extra_amount" value="<?php if(isset($s_array1)){ echo $s_array1[$i]; }?>"><br>
			</td>
...
<?php } ?>
	...
	<?php
				...
				<tr>
				<td hidden>
				<input type="text" readonly id="amenity_fees_id" name="amenity_fees_id[]" class="form-control" value = "<?php if(isset($s_array5)){ echo $s_array5[$i]; }?>"></td>
	...
	<table id="general_concessionTable" class="table custom-table" cellspacing="0" >
			<thead>
						...
						<?php
						$ctselect1="SELECT * FROM pay_transport_fees WHERE student_id='$student_name1' AND standard ='$standard1' AND academic_year='$currentAcademicYear' AND school_id = '$school_id' AND status=0";
						$ctresult1=$mysqli->query($ctselect1);
						if($ctresult1->num_rows>0){
							$i=1;
							$paytransportfeesid = '0';
			...
			<?php if (isset($paytransportfeesid)){  
				...
				$currentAcademicYear = $_SESSION['academic_year']; 
				$school_id =$_SESSION['school_id']; 
				$ctselect="SELECT * FROM pay_transport_fees WHERE student_id='$student_name1' AND standard ='$standard1' AND academic_year='$currentAcademicYear' AND school_id = '$school_id' AND status=0";
				$ctresult=$mysqli->query($ctselect);
			if($ctresult->num_rows>0){
			$i=1;
			while($ct=$ctresult->fetch_assoc()){
