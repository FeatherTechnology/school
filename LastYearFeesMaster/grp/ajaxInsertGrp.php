...
$selectClass=$con->query("SELECT * FROM last_year_fees_master WHERE academic_year = '".$academic_year."' AND medium = '".$medium."' AND student_type = '".$student_type."'
...
if($grp_academic_year != '' && $grp_medium != '' && $grp_student_type != '' && $grp_standard != '' && $grpc_particulars != ''  && $grpc_amount != '' && $grpc_date != '' && $grp_Status == 0){ 
	...
	$updateClass=$con->query("UPDATE last_year_fees_master SET status=0 WHERE academic_year = '".$academic_year."' AND medium = '".$medium."' AND student_type = '".$student_type."' AND standard = '".$standard."'
	...
	if($last_year_fees_master_id>0){
		$updateClass1=$con->query("UPDATE last_year_fees_master SET academic_year = '".$academic_year."', medium = '".$medium."', student_type = '".$student_type."', 
	    ...
	    $insertClass=$con->query("INSERT INTO last_year_fees_master(academic_year,medium,student_type,standard,grp_particulars,grp_amount,grp_date,insert_login_id,grp_status) VALUES('".strip_tags($academic_year)."','".strip_tags($medium)."',
