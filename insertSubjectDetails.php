...
if($paper_name != '' && $max_mark != '' && $class_id != '' && $classStatus == 0){
	...
	if($subject_id>0){
	    ...
	    $insertClass=$con->query("INSERT INTO subject_details(paper_name, max_mark, class_id, insert_login_id) VALUES('".strip_tags($paper_name)."', '".strip_tags($max_mark)."','".strip_tags($class_id)."', '".strip_tags($insert_login_id)."')");
	    if($insertClass == true){
		    $message="Subject Details Added Succesfully";
	    }
    }
