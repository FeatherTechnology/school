...
$selectClass=$mysqli->query("SELECT * FROM fees_master WHERE academic_year = '".$academic_year."' AND medium = '".$medium."'
AND grp_particulars = '".$grp_particulars."' AND grp_amount = '".$grp_amount."' AND standard = '".$standard."' ");
...
if($grp_academic_year != '' && $grp_medium != '' && $grp_standard != '' && $grpc_particulars != ''  && $grpc_amount != '' && $grp_Status == 0){ echo "1";
	...
	$updateClass=$mysqli->query("UPDATE fees_master SET status=0 WHERE academic_year = '".$academic_year."' AND medium = '".$medium."' AND standard = '".$standard."'
	AND grp_particulars = '".$grp_particulars."' AND grp_amount = '".$grp_amount."' ");
	...
	if($fees_id>0){
		...
		AND grp_particulars = '".$grp_particulars."' AND grp_amount = '".$grp_amount."' WHERE fees_id='".$fees_id."' ");
		if($updateClass == true){
		    $message="Fees Details Updated Succesfully";
	    }
    }
	else{ 
	    $insertClass=$mysqli->query("INSERT INTO fees_master(academic_year,medium,standard,grp_particulars,grp_amount,insert_login_id,grp_status) VALUES('".strip_tags($academic_year)."','".strip_tags($medium)."',
		'".strip_tags($standard)."','".strip_tags($grp_particulars)."','".strip_tags($grp_amount)."','".strip_tags($insert_login_id)."', '1')");
	    if($insertClass == true){
