...
$(document).ready(function () {
  ...
  let selectedFeeType = ''; // NEW


  $("#paid_fees_details").on('click', '.edit_payfees', function () {
    // Store the selected fees ID when the edit icon is clicked
    selectedFeesId = $(this).closest('tr').find('.fees_id').val();
    selectedFeeType = 'admission'; // admission fee
    $('#reason').val(''); // Clear any previous reason
    $('#reasonCheck').hide(); // Hide validation initially
    $('#rejectpobtn').prop('disabled', false); // Enable submit button
  });
  $("#paid_fees_details").on('click', '.edit_transfees', function () {
    // Store the selected fees ID when the edit icon is clicked
    selectedFeesId = $(this).closest('tr').find('.fees_id').val();
    selectedFeeType = 'transport'; // transport fee
    $('#reason').val(''); // Clear any previous reason
    $('#reasonCheck').hide(); // Hide validation initially
    $('#rejectpobtn').prop('disabled', false); // Enable submit button
  });
  $("#paid_fees_details").on('click', '.edit_lastpayfees', function () {
    // Store the selected fees ID when the edit icon is clicked
    selectedFeesId = $(this).closest('tr').find('.fees_id').val();
    selectedFeeType = 'last_year'; // transport fee
    $('#reason').val(''); // Clear any previous reason
    $('#reasonCheck').hide(); // Hide validation initially
    $('#rejectpobtn').prop('disabled', false); // Enable submit button
  });
  // Handle form submission
  $('#edit_student__creation').on('submit', function (event) {
    ...
    // Call your custom function to handle the update
    editFeesCollection(selectedFeesId, selectedFeeType, reason);
...
function editpayFees(feesId, studentId) {
  var url = "pay_fees&pagename=feescollection&upd=" + studentId + "&feesid=" + feesId;
  window.location.href = url;
}
...
function editTranspayFees(feesId, studentId) {
  var url = "transport_fees&pagename=feescollection&upd=" + studentId + "&feesid=" + feesId;
  window.location.href = url;
}
...
function editLastYearFees(feesId, studentId) {
  var url = "last_year_fees_pay&pagename=feescollection&upd=" + studentId + "&feesid=" + feesId;
  window.location.href = url;
}
...
function editFeesCollection(feesId, feeType, reason) {
  var payfeestudentId = $('#getStudID').val();

  $.ajax({
    url: 'FeesCollectionFile/feesCollection/submitreason.php',
    type: 'POST',
    dataType: 'json',
    data: {
      fees_id: feesId,
    ...
    success: function (response) {
      if (response.status == 1) {
        ...
        switch (response.table) {
          case 'admission_fees':
            editpayFees(feesId, payfeestudentId);
            break;
          case 'transport_admission_fees':
            editTranspayFees(feesId, payfeestudentId);
            break;
          case 'last_year_fees':
            editLastYearFees(feesId, payfeestudentId); 
            break;
        }
