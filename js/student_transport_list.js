...
$(document).ready(function () {
    ...
    $('#transport_list_table_view').click(function(){
        let academicyear = $('#academic_year').val();
        ...
        if(academicyear !='0' && stdMedium !='0'){
            $.ajax({
                ...
                url: 'reports/student_transport_report/getStudentTransportListTable.php',
                success: function(response){
                    $('#listCard').show();
                    $('#showStudentTransportList').empty();
                    $('#showStudentTransportList').html(response);
                }
            })
        }else{
            $('#showStudentTransportList').empty();
...
function getStandardList() { //Getting standard list from database.
    ...
    success: function (response) {
        $('#standard').empty();
        $('#standard').append("<option value='0'>All</option>");
        for (var i = 0; i < response.length; i++) {
