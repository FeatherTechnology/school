...
//Vendor Check
  ...
  $("#submitpurchaseorder").click(function () {
    ...
    if(vendorError == true && shiptoError ==true && dateError == true && po_check == true){
      return true;
    }
    else
    {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var appendTxt =
        ...
        "<td><input type='hidden' class='form-control maxquantity inputs' readonly  placeholder='0.0' /><input type='number' onkeydown='javascript: return event.keyCode == 69 ? false : true' tabindex='14' class='form-control qty'  name='qty[]' placeholder='0' id='qty' /></td>" +
        "<td><input type='text' tabindex='15' class='form-control rate' name='rate[]' id='rate' placeholder='0.0' /></td>" +
        "<td><input type='text' tabindex='17' class='form-control totval inputs' readonly id='totval' name='totval[]' placeholder='0.0' /></td>" +
  ...
  $(document).on('change', '.qty, .rate', function () {
    ...
    $('#sub_quantity').val(subQuantitySum);
    $('#unit_amount').val(unitAmountSum);
    $('#total_amount').val(totalAmountSum);
  });
