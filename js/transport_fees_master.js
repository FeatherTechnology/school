...
$(document).ready(function () {	
  ...
  $(document).on("click", "#Submit_Group", function () { 
   ...
   if(this.id == "Submit_Group") {
        ...
        var transport_fees_master_id=$("#transport_fees_master_id").val();
        var date=$("#grp_date").val(); 
        var ajaxUrl = 'TransportFeesMaster/grp/ajaxInsertGrp.php';
    ...
    if(particulars!="" && amount!="" && date!="") {
        $.ajax({
            url: ajaxUrl,
            type: 'POST',
            data: {"transport_fees_master_id":transport_fees_master_id,"particulars":particulars,"amount":amount,"date":date,"academic_year":academic_year,"insert_login_id":insert_login_id,"medium":medium, "student_type":student_type, "standard":standard},
            cache: false,
      ...
      function resetstockinfotable(){
        ...
        var standard=$("#standard").val();
          $.ajax({
          url: 'TransportFeesMaster/grp/ajaxResetGrpTable.php',
          type: 'POST',
    ...
    $("#grp_date").keyup(function(){
      ...
      $("#grp_dateCheck").hide();
       ...
       $("body").on("click","#edit_grp",function(){
           var transport_fees_master_id=$(this).attr('value');
           $("#transport_fees_master_id").val(transport_fees_master_id); 
           $.ajax({
                   url: 'TransportFeesMaster/grp/ajaxEditGrp.php',
                   type: 'POST',
                   data: {"transport_fees_master_id":transport_fees_master_id},
                   dataType: 'json',
                   cache: false,
                   success:function(response){
     ...
     $("body").on("click","#delete_grp", function(){ 
       ...
       if(isok==false){
         return false;
           ...
           $.ajax({
               url: 'TransportFeesMaster/grp/ajaxDeleteGrp.php',
               type: 'POST',
               data: {"transport_fees_master_id":transport_fees_master_id},
               cache: false,
               success:function(response){ 
