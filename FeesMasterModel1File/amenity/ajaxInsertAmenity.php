...
if($grp_academic_year != '' && $grp_medium != '' && $grp_student_type != '' && $grp_standard != '' && $grpc_particulars != ''  && $grpc_amount != '' && $grpc_date != '' && $grp_Status == 0){ echo "1";
	...
	if($fees_id>0){
	    ...
	    $insertClass=$con->query("INSERT INTO fees_master(academic_year,medium,student_type,standard,amenity_particulars,amenity_amount,amenity_date,insert_login_id, amenity_status,user_id,school_id) VALUES('".strip_tags($academic_year)."','".strip_tags($medium)."',
		'".strip_tags($student_type)."','".strip_tags($standard)."','".strip_tags($amenity_particulars)."','".strip_tags($amenity_amount)."','".strip_tags($amenity_date)."','".strip_tags($insert_login_id)."','1','".strip_tags($user_id)."','".strip_tags($school_id)."')");
	    if($insertClass == true){
