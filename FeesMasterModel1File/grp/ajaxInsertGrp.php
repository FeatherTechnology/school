...
if($grp_academic_year == $academic_year && $grp_medium == $medium && $grp_student_type == $student_type && $grp_standard == $standard && $grpc_particulars == $grp_particulars  && $grpc_amount == $grp_amount && $grpc_date == $grp_date && $grp_Status == 1){ 
	...
	$updateClass=$con->query("UPDATE `group_course_fee` SET `grp_particulars`='$grp_particulars',`grp_amount`='$grp_amount',`grp_date`='$grp_date',`status`='1' WHERE grp_particulars = '".$grp_particulars."' AND grp_amount = '".$grp_amount."' AND grp_date = '".$grp_date."' ");
	$message="Fees Details Added Succesfully";

}else{ 
	if($grp_course_id>0){
		$updateClass1=$con->query("UPDATE `group_course_fee` SET `grp_particulars`='$grp_particulars',`grp_amount`='$grp_amount',`grp_date`='$grp_date',`status`='1' WHERE grp_course_id='".$grp_course_id."' "); 
		...
		$insertGrpFees = $con->query("INSERT INTO `group_course_fee`(`fee_master_id`, `grp_particulars`, `grp_amount`, `grp_date`) VALUES ('".strip_tags($fee_master_last_id)."','".strip_tags($grp_particulars)."','".strip_tags($grp_amount)."','".strip_tags($grp_date)."' )");
