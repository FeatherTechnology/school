...
if($extra_academic_year == $academic_year && $extra_medium == $medium && $extra_student_type == $student_type && $extra_standard == $standard && $extrac_particulars == $extra_particulars && $extrac_amount == $extra_amount && $extrac_date == $extra_date && $extra_Status == 1){ ;
	...
	$updateClass=$con->query("UPDATE extra_curricular_activities_fee SET extra_particulars = '".$extra_particulars."', extra_amount = '".$extra_amount."', extra_date = '".$extra_date."', `status`='1' WHERE extra_particulars = '".$extra_particulars."' AND extra_amount = '".$extra_amount."' AND extra_date = '".$extra_date."' ");
	$message="Fees Details Added Succesfully";

}else{ 
	if($extra_fee_id>0){
		$updateextra=$con->query("UPDATE extra_curricular_activities_fee SET extra_particulars = '".$extra_particulars."', extra_amount = '".$extra_amount."', extra_date = '".$extra_date."', `status`='1' WHERE extra_fee_id='".$extra_fee_id."' ");
		if($updateextra == true){
		...
		if(mysqli_num_rows($feeMasterrowcnt) > 0){
			...
			$insertClass=$con->query("UPDATE `fees_master` SET `extra_status`='1',`update_login_id`='$user_id',`updated_date`='$curdate' WHERE `fees_id`='$fee_master_last_id' ");

		}else{
			$insertClass=$con->query("INSERT INTO fees_master(academic_year,medium,student_type,standard,extra_status,insert_login_id,school_id) VALUES('".strip_tags($academic_year)."','".strip_tags($medium)."', '".strip_tags($student_type)."','".strip_tags($standard)."','1','".strip_tags($user_id)."','".strip_tags($school_id)."')");
			$fee_master_last_id = mysqli_insert_id($con);
			
		}

		$insertExtraFees = $con->query("INSERT INTO `extra_curricular_activities_fee`( `fee_master_id`, `extra_particulars`, `extra_amount`, `extra_date`) VALUES ('".strip_tags($fee_master_last_id)."','".strip_tags($extra_particulars)."','".strip_tags($extra_amount)."','".strip_tags($extra_date)."' )");
