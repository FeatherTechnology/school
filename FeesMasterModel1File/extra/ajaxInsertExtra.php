...
if($extra_academic_year == $academic_year && $extra_medium == $medium && $extra_student_type == $student_type && $extra_standard == $standard && $extrac_particulars == $extra_particulars && $extrac_amount == $extra_amount && $extrac_date == $extra_date && $extra_Status == 1){ ;
	...
	$updateClass=$mysqli->query("UPDATE extra_curricular_activities_fee SET extra_particulars = '".$extra_particulars."', extra_amount = '".$extra_amount."', extra_date = '".$extra_date."', `status`='1' WHERE extra_particulars = '".$extra_particulars."' AND extra_amount = '".$extra_amount."' AND extra_date = '".$extra_date."' ");
	$message="Fees Details Added Succesfully";

}else{ 
	if($extra_fee_id>0){
		$updateextra=$mysqli->query("UPDATE extra_curricular_activities_fee SET extra_particulars = '".$extra_particulars."', extra_amount = '".$extra_amount."', extra_date = '".$extra_date."', `status`='1' WHERE extra_fee_id='".$extra_fee_id."' ");
		if($updateextra == true){
		...
		$insertExtraFees = $mysqli->query("INSERT INTO `extra_curricular_activities_fee`( `fee_master_id`, `extra_particulars`, `extra_amount`, `extra_date`) VALUES ('".strip_tags($fee_master_last_id)."','".strip_tags($extra_particulars)."','".strip_tags($extra_amount)."','".strip_tags($extra_date)."' )");
