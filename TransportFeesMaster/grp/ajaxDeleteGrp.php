...
$ctqry=$con->query("SELECT * FROM transport_fees_master WHERE grp_particulars = '".$transport_fees_master_id."' ");
...
{ 
	$delct=$con->query("UPDATE transport_fees_master SET status = 1 WHERE transport_fees_master_id = '".$transport_fees_master_id."' ");
	if($delct){
