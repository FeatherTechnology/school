...
if($subpaper_name != '' && $subclass_id != '' && $classStatus == 0 && $maxMark == $max_mark){
	...
	$updateClass=$mysqli->query("UPDATE subject_details SET paper_name='$paper_name', max_mark='$max_mark', status = 0, update_login_id = '$insert_login_id', updated_date = '$curdate' WHERE paper_name='".$paper_name."' AND class_id = '".$class_id."' ");
	...
	if($subject_id>0){
		...
		$insertClass=$mysqli->query("INSERT INTO subject_details(class_id, paper_name, max_mark, insert_login_id) VALUES('".strip_tags($class_id)."', '".strip_tags($paper_name)."', '".strip_tags($max_mark)."', '".strip_tags($insert_login_id)."')");
		if($insertClass == true){
