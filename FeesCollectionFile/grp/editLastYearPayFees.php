...
if (isset($_POST['fees_id'])) {
    ...
    $editAdmissionFeesQry = $connect->query("SELECT * FROM `last_year_fees` WHERE `id` ='$feesid' ");
    $editAdmissionFees = $editAdmissionFeesQry->fetch(PDO::FETCH_ASSOC);
    // Fetch fee details
    $editAdmissionFeesDetailsQry = $connect->query("SELECT * FROM `last_year_fees_details` WHERE `admission_fees_ref_id` ='$feesid'");
    $groupFeesHtml = '';
    $extraFeesHtml = '';
    $amenityFeesHtml = '';
    $transportFeesHtml = '';
...
<td><input type="text" class="form-control amenityAmtremarks" name="amenityAmtremarks[]" value="' . $feeDetail['remarks'] . '"></td>
            ...
            $transportFeesHtml .= '<tr>
                <td>
                    <input type="hidden" class="form-control" name="areaCreationId[]" value="' . $feeDetail['fees_master_id'] . '">
                    <input type="hidden" class="form-control" name="particularId[]" value="' . $grpFee['particulars_id'] . '">
                    ' . $grpFee['particulars'] . '
                </td>
                <td><input type="number" class="form-control transportfeeslastamnt" name="transportFeeAmnt[]" value="' . $transportAmount . '" readonly></td>
