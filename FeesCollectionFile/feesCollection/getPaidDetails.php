...
<table class="table custom-table" id="paid_details_table">
    ...
    <tbody>
        ...
        $getAdmissionFees = $connect->query("SELECT af.id, af.receipt_date, af.receipt_no, af.academic_year, GROUP_CONCAT( CASE WHEN(afd.fees_table_name ='grptable') 
        THEN gcf.grp_particulars ELSE CASE WHEN(afd.fees_table_name ='extratable') THEN ecaf.extra_particulars ELSE CASE WHEN(afd.fees_table_name ='amenitytable') THEN aff.amenity_particulars END END END ORDER BY afd.fees_table_name SEPARATOR ', ') as particulars, af.fees_collected  
        ...
        ?>
                <tr id="pay_fee_table" class="pay_fee_table">
                    ...
                    <td>
                        ...
                        <?php if ($role == 1): ?>
                            <a class="edit_payfees" data-toggle='modal' data-target='#rejectModal' class='btn rejectpo' id='rejectStud' title='Edit student'><span class='icon-border_color'></span></a>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php
...
WHERE lyf.admission_id = '$student_id' && lfd.fee_received != '0' && lyf.academic_year ='$currentAcademicYear' GROUP BY lyf.receipt_no ORDER BY lyf.id DESC ");
        if ($getLastAdmissionFees->rowCount() > 0) {
            ...
            ?>
                <tr id="pay_fee_table" class="pay_fee_table">
                    ...
                    <td>
                        <a class='printpo'><span class='icon-print'></span>&nbsp;</a> &nbsp;
                        <a class="edit_lastpayfees" data-toggle='modal' data-target='#rejectModal' class='btn rejectpo' id='rejectStud' title='Edit student'><span class='icon-border_color'></span></a>
                    </td>
                </tr>
            <?php
            }
        ...
        $getTransportPaidDetails = $connect->query("SELECT taf.id, taf.receipt_date, taf.receipt_no, taf.academic_year, GROUP_CONCAT(acp.particulars ORDER BY acp.particulars SEPARATOR ', ') AS particulars, SUM(tafd.fee_received) AS total_received
        ...
        WHERE taf.admission_id = '$student_id' && taf.academic_year = '$currentAcademicYear' && tafd.fee_received != '0' GROUP BY taf.receipt_no order by taf.id DESC ");
        if ($getTransportPaidDetails->rowCount() > 0) {
            ...
            ?>
                <tr id="pay_trans_fee_table" class="pay_trans_fee_table" style="display:none;">
                    ...
                    <td><?php echo $transportfeeInfo['total_received']; ?></td>
                    <td>
                        <a class='print_transport_fees'><span class='icon-print'></span>&nbsp;</a> &nbsp;
                        <?php if ($role == 1): ?>
                            <a class="edit_transfees" data-toggle='modal' data-target='#rejectModal' class='btn rejectpo' id='rejectStud' title='Edit student'><span class='icon-border_color'></span></a>
                        <?php endif; ?>
                    </td>
                </tr>
        <?php
